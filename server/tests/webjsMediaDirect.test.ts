...
const clientWith = (impl: (id: string) => any) => ({
  pupPage: { evaluate: async (_fn: any, id: string) => impl(id) },
});
...
describe('direct media download', () => {
  ...
  it('never throws — no page, no id, exploding page', async () => {
    ...
    expect((await downloadMediaDirect(clientWith(() => ({})), '')).reason).toBe('no message id');
...
describe('the fix avoids the poisoned lookup', () => {
  ...
  it('matches by string on id._serialized', () => {
    expect(SRC).toContain("(m?.id?._serialized ?? '') === msgId");
  });
