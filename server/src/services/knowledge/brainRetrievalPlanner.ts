...
- **intent = "introspective"** — questions about Brain, the user, or the tenant as an entity ("who are you", "what can you access", "what do we do", "tell me about my company"). List the foundational FACL org_doc titles in \`faclTitles\` (Company Identity, Org Chart, OKR Tree, Strategy Decision Log) so Brain sees their FULL body, not just previews. **For OPERATIONAL questions about Brain's own behaviour ("how do you decide what becomes an open item?", "when do you call me?", "how do follow-ups work?", "what triggers an email to my delegatee?", "how do you handle deadlines?") ALSO add "MyOS Operations Manual" to \`faclTitles\` — it is the source-of-truth doc for Brain's design rules.**
