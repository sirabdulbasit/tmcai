...
async function buildCriticalItems(userId: number, clientNumber: string): Promise<string> {
  ...
  // Fetch star-cadence activity for every item so we can tell the user
  // "I texted you 2× about this — no reply yet" / "I called you, no
  // answer; voicenote follow-up sent". Bulk lookup keeps it cheap.
  let cadenceByItem = new Map<string, string>();
  try {
    const { getCadenceSummaryForItem } = await import('./triage/starCadenceService');
    const summaries = await Promise.all(individualItems.map(async (it) => {
      const s = await getCadenceSummaryForItem(it.id).catch(() => null);
      return [it.id, s] as const;
    }));
    for (const [id, s] of summaries) {
      if (!s || s.totalScheduled === 0) continue;
      const note = renderCadenceNote(s);
      if (note) cadenceByItem.set(id, note);
    }
  } catch { /* cadence is optional context */ }
  ...
  const itemLines = individualItems.map(item => {
    ...
    const cadence = cadenceByItem.get(item.id) ? `\n   ${cadenceByItem.get(item.id)}` : '';
    ...
    return `${priority} **${item.title}**${score}${due}${delegatee}${source}${entityCtx}${erg}${cadence}${suggestions}\n   Status: ${item.status} | Type: ${item.type}`;
  }).join('\n\n');

  return (situationBlocks + itemLines).trim();
}

function renderCadenceNote(s: {
  totalScheduled: number; sent: number; pending: number; answered: number;
  skipped: number; expired: number; lastSentAt: Date | null; lastChannelUsed: string | null;
}): string {
  if (s.totalScheduled === 0) return '';
  if (s.answered > 0) {
    return `*Brain pinged you ${s.sent}× via WhatsApp — you replied.*`;
  }
  if (s.sent > 0 && s.pending > 0) {
    const ch = channelLabel(s.lastChannelUsed);
    return `*Brain ${ch} you ${s.sent}× — no response yet, ${s.pending} more queued.*`;
  }
  if (s.sent > 0 && s.pending === 0) {
    const ch = channelLabel(s.lastChannelUsed);
    return `*Brain ${ch} you ${s.sent}× via WhatsApp — no response, cap reached.*`;
  }
  if (s.pending > 0) {
    return `*Brain has ${s.pending} WhatsApp ping${s.pending === 1 ? '' : 's'} scheduled for this.*`;
  }
  if (s.skipped > 0 && s.sent === 0) {
    return `*Brain stopped pinging — you took action.*`;
  }
  return '';
}

function channelLabel(channel: string | null): string {
  if (!channel) return 'pinged';
  if (channel.includes('call')) return 'called';
  if (channel.includes('voice')) return 'voice-noted';
  return 'texted';
}
