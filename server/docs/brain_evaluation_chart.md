...
**Version:** v3.5 · **Last updated:** 2026-08-10 22:30 PKT
...
|---|---|---|
| v3.5 | 2026-08-07 23:55 | **D-13/D-14/D-15 deployed.** First deploys driven by READING the owner's actual WhatsApp conversation rather than waiting for him to report a bug: DEF-089 (agent routing dead on every turn), DEF-091 (6 unscoped cross-tenant queries), DEF-092 (77 of 102 markers stripped to silence), DEF-093 (his answer to Brain's own question became a task), DEF-094 ("immediate" rejected as a deadline). Tests 1461 → 1495. |
...
|---|---|---|---|---|---|---|
| **D-22** | **08-11 20:05 PKT** | `41dfdff` | DEF-129 (+ DEF-130 logged) | **Predicted:** removal and mark-wrong stop writing statuses the lifecycle does not know; a refused delegation stops leaving a delegatee behind; six items stranded in DONE become movable; withdrawn work leaves the active list. **Risk:** adding a second terminal state without updating every "active items" filter would make cancelled work keep nagging — closed by one shared INACTIVE_STATUS_VALUES plus explicit CANCELLED in the raw-SQL chasing paths. Second risk: the learning gate that blocks junk senders queried `status: 'closed'` and would have silently stopped matching. | **FOUR LIVE TESTS, ALL WITH DATABASE READ-BACK.** Brain delegation TRIAGED→DELEGATED accepted with delegatee set and one ledger row. **Refused delegation NEW→DELEGATED rejected with `delegatee_name`, `delegatee_email` and `delegatee_id` all NULL** — the HIGH review finding closed and proven, not argued. Cancellation TRIAGED→CANCELLED accepted; restoration CANCELLED→TRIAGED accepted, so a withdrawal is genuinely reversible. DONE normalisation moved 6 rows with 6 audit rows, and a re-run changed nothing. `/metrics` now reachable. **NOT verified: Brain's removal PHRASING** — it denied a capability it has (DEF-130), so removal is proven at the service layer but not through conversation. | **PROGRESS — Family A closed at the data layer; one new conversational defect logged, not papered over** |
| **D-21** | **08-11 19:20 PKT** | `fe824d5` | DEF-127, DEF-128 | **Predicted:** accepted status transitions stop rolling back, so the Open Items status control and Brain's mark_open_item_done/delegate_open_item work again, and every transition leaves durable user-scoped audit. **Risk:** `transitionStatus` previously ALWAYS threw, so anything that silently depended on it failing would start succeeding; migration touches production schema. Mitigated by applying the migration before restart, proving idempotency by re-running it, and accepting on dedicated test items only. | **BOTH PATHS VERIFIED LIVE.** UI `POST /open-items/:id/status` (real HTTP, real auth) NEW→TRIAGED → one accepted row, user 2, actor `user:2`. Brain, a real LLM turn, TRIAGED→CLOSED → one accepted row. Migration verified on production: `user_id` NOT NULL, three indexes, 0 rows, and a SECOND run of the same file returned 0 errors. **The ledger paid for itself within the hour: it exposed DEF-128** — Brain answered *Marked "…" done.* while the audit row said `rejected — approval required` and the item was still NEW. Both Brain call sites discarded the transition result; fixed and re-verified, Brain now returns an honest refusal marker. | **PROGRESS — broken Action capability restored, and a fabricated-completion defect caught by the new evidence** |
| **D-20** | **08-11 18:16 PKT** | `ccbdfda` | MEM-005 (+ DEF-127 logged) | **Predicted:** the two non-wiki embedding paths stop calling an endpoint Google retired 28 days ago, and every future provider change lands in ONE place instead of three. The chunk repair sweep starts selecting retired-model rows it was previously blind to, and a tenant whose chunks are all stale is selected at all. A dead open-item path that threw a TypeError on every item creation since 2026-05-18 is removed, along with a route that answered 500 for its whole life. **Risk:** the wiki path is the one with 10,476 healthy rows and it now runs through new code — the danger is a behaviour change there, not in the two broken paths. Mitigated by keeping storage and retrieval untouched and asserting the request shape (model + outputDimensionality 768), unit normalisation, and that current-model rows are still skipped; DEF-118's ten assertions stay green. Second risk: removing the `open_items` health component changes `/admin/system-health` output by one entry that was permanently 'ok'. **Judged by:** `wiki_pages` stub-768 continuing to fall from 2,039; new rows persisting `gemini-embedding-001` at dim 768; NO new `embedding_provider_degraded` findings against a healthy provider. **NOT judged by chunks** — that table is empty (0 rows), so the chunk half of this change is unfalsifiable in production and is reported as such rather than counted as a win. | **WIKI PATH VERIFIED LIVE.** A real sweep through the NEW shared provider returned `attempted 25, embedded 25, degraded false` against production, and the persisted rows were inspected directly: `embedding_model = gemini-embedding-001`, `vector_dims = 768`, `L2 norm = 1.0000` — model, width and normalisation all confirmed in the database, not inferred from a green build. Stale count fell **2,039 → 2,014** and current rows rose **10,476 → 10,501**; monotonic, as required. Boot clean; the two log errors present are pre-existing (Secret Manager fallback, and a 22021 UTF-8 raw-query fault timestamped BEFORE the restart — DEF-085 class, untouched by this change). Also verified in the built artifact: exactly ONE `generativelanguage` fetch URL now exists in dist, inside the provider, built from the shared model constant; the stale compiled output of the deleted service was removed after confirming nothing imports it. **CHUNK HALF NOT VERIFIABLE — stated, not counted:** `chunks` holds 0 rows, so the `IS DISTINCT FROM` fix and the scheduler predicate are asserted by 21 tests but have no production data to converge. **OPEN-ITEM RETIREMENT VERIFIED structurally** (module gone, route absent from the Express table, 3 phantom registry entries removed) but its user-visible effect was a 500 nobody called, so there is nothing to observe. **Convergence, evidence-based, NOT a promise:** the scheduled sweep takes 200/run hourly, so the remaining 2,014 need roughly **11 hourly cycles** if the provider stays healthy and no new stale rows arrive — the 25-row manual run is the only measured data point so far. | **PROGRESS — wiki verified live; chunk half unfalsifiable (empty table); no regression** |
| **D-19** | **08-11 11:47** | `HEAD` | DEF-119 | **Predicted:** the stale-prompt guard starts covering `preactive_due_nudge`, which is 14 of the last 40 prompts and 0% covered until now — so a reminder whose item closes while it waits through quiet hours stops going out. **Risk:** the resolver returning a junk id would make the guard treat a live item as deleted and drop a real reminder. Seven null cases asserted for exactly that, and the guard still fails OPEN on a broken lookup. **Judged by:** a `preactive_due_nudge` prompt whose item closes before dispatch appearing in `dropped prompts whose subject is finished` — not by tests. | Deployed. **NOT VERIFIED** — needs that sequence to occur naturally; prompt #322 and #323 are queued now and their items are still open, so there is nothing to drop yet. Suite 1731 passed / 0 failed after the change; the fix also repaired 8 DEF-115 assertions my own docblock had broken. | **UNVERIFIED (pending live sequence)** |
| **D-18** | **08-11 11:24** | `414d8cb` | DEF-115, DEF-116 (log rows only — code shipped 08-10) | **Predicted:** nothing behavioural. This row exists because two fixes shipped yesterday with no ledger entry (BLD-006), so their real verification state was invisible to the chart. **Risk:** none to the running system; the risk it removes is a recurrence being misread as a first occurrence. | DEF-115 **VERIFIED LIVE** on 08-10 (`dropped prompts whose subject is finished, promptIds ["318"]` at 18:22:50). DEF-116 **DEPLOYED, UNVERIFIED** — needs a live delegation naming an unknown contact. Both now carry rows. | **NEUTRAL (bookkeeping)** |
| **D-17** | **08-11 11:24** | `414d8cb` | DEF-117, DEF-118 | **Predicted:** (117) when Brain sends a reminder and the owner replies to it, Brain can see the reminder and answers about it instead of asking what he means — the 05:52 turn scored 71 for exactly this. (118) wiki retrieval stops searching 63% of memory with meaningless stub vectors, and `embedding_provider_degraded` stops firing hourly against a healthy provider. **Risk:** (117) writes to live session history — mitigated by `sent`/`partial` only, so a message the owner never received can never become something Brain believes it said; single atomic UPDATE so no concurrent append is lost. (118) removing a skip could make the sweep re-embed all 12,483 pages every 50 minutes and rate-limit itself — the hash check is asserted to still skip unchanged current-model pages. **Judged by:** (117) his next reply to a reminder; (118) `embedding_model` distribution reaching 0 stub rows and the finding ceasing. | **DEF-118 VERIFIED LIVE 11:31** — sweep returned `attempted 5, embedded 5, degraded false` against the same rows that returned `0/200` for 21 hours; stub count began falling (7,893 → 7,887 → backfill running). Provider was proven healthy (HTTP 200) BEFORE the code changed, so the fix was aimed at the writer and not guessed. **DEF-117 DEPLOYED, NOT VERIFIED** — needs the owner to reply to a proactive message; asserted by 15 tests but no live exercise yet. Server up 06:24:20 UTC, WhatsApp ready 06:24:34, `unstable restarts 0`. | **PROGRESS (1 of 2 verified live)** |
| **D-16** | **08-10, ten deploys** (`ed9daf4` → `439bbae`) | `439bbae` | DEF-107, 108, 108b, 109, 110, 111, 112, 113, 114 | **Predicted:** the output layer stops destroying correct answers (107, 114); Brain can see and report its own sends (108, 108b); "delegate to brain" persists and stops nagging (109); images readable (110); the fabrication guard stops missing *"I've just sent it"* (111); four failed writes per turn stop (112); Brain stops denying a capability it has (113). **Risk:** three of these touch the empty-promise guard, which DEF-041 shows is the most dangerous surface in the system — every change was checked against DEF-041's verbatim sentence, and all three fail CLOSED (unknown intent still blocks). **Judged by:** send an image; ask *"what did you send today"*; ask a capability question and expect an answer rather than a denial. | **TWO VERIFIED LIVE.** DEF-112: `spend record (legacy) failed` **302 → 0** since deploy, confirmed in the log. DEF-108: `brain_action_artifacts` **20 → 22 rows**, two `notify_via_whatsapp/succeeded` written post-deploy — the ledger is fed for the first time. The other seven are DEPLOYED, UNVERIFIED — none has been exercised by owner traffic yet. **Process note:** two deploy mistakes, both self-caught — a `;` instead of `&&` let build+restart run after a FAILED merge, and one merge ran in the worktree instead of production and nearly got reported as shipped. HEAD is now checked before and after every merge. | **PROGRESS (2 of 9 verified)** |
...
| D-12 | **08-07 19:09 PKT** | `92f28fe` | Stage 5 (notify) · DEF-089 logged | **Predicted:** Brain can reach the owner on WhatsApp unprompted, so deploy reports stop depending on him opening a session. **Risk:** the send path touches the fragile layer; an out-of-process sender would disturb the live session. **Judged by:** a real message arriving on his phone AND a `brain_user_messages` row with `status='sent'` — written is not delivered. | **VERIFIED on both halves.** Ledger row `#18789` `kind=deploy_report channel=text status=sent error=-`, and the **watcher independently observed it** (`TOLD #18789 sent kind=deploy_report via=text`) — stage 5 confirmed by stage 1, which is the first time two parts of this loop have corroborated each other. **The first design was wrong and the watcher caught it:** the standalone CLI built its own webjs client on the live LocalAuth dir and triggered `probe_fail → bounded_reinit` at 19:02:15; channel went `connected → liveness_failed → connected` over ~45s and the message never landed. Rebuilt to send over loopback; no `probe_fail` on the second attempt. **Still pending owner acceptance — he has not confirmed the message arrived on his phone.** | **PROGRESS** (pending owner acceptance) |
| D-15 | **08-07 23:55 PKT** | `ec729e8` | DEF-093, DEF-094 | **Predicted:** an answer to Brain's own question can no longer become a task, and "immediate"/"asap"/"abhi" work as deadlines. **Risk:** low but real — narrowing the piggyback extractor could suppress a genuine directive riding alongside an answer (A6), so that case is asserted explicitly rather than assumed. **Judged by:** answer a priority/deadline prompt and confirm no junk item appears; set a due date of "immediate" and confirm it lands on today. | **Both traced from his real conversation, not hypothesised.** DEF-093's correlation was already CORRECT (`sideEffect: applied`, 17:09:26) — the damage came 1s later from a second mechanism re-reading the same words. Root cause was two implementations of one rule. Fixes confirmed in `dist`, not just source. **NOT yet exercised live** — no prompt has been answered since the deploy. Notify correctly declined on **quiet hours** (exit 3, deliberate, not a failure). | **PROGRESS — pending live acceptance** |
| D-14 | **08-07 23:31 PKT** | `74981b2` | DEF-092 | **Predicted:** Brain stops reciting canned lines and stops going silent. **Risk:** an LLM on the reply path can fabricate; constrained so the marker is the ONLY source of fact, and it can never return silence or brackets. **Judged by:** trigger a status reply and see a real sentence rather than `[noted]`. | Measured before fixing: **25 canned / 77 silent / 3 raw brackets**. Silence was the majority — a user watching their request vanish concludes Brain is broken. Deployed and present in `dist`. 9 assertions pin the two invariants (never brackets, never silence). **Live phrasing NOT yet observed by the owner.** | **PROGRESS — pending live acceptance** |
| D-13 | **08-07 19:45 PKT** | `55dd4e1` | DEF-089, DEF-090, DEF-091 | **Predicted:** agent routing stops failing on every turn; cross-tenant reads/writes closed. **Risk:** low — removing references to columns that do not exist. **Judged by:** `Agent detection failed` disappears from the log. | **VERIFIED on the log:** zero occurrences after restart, was every single turn before. Root cause `agents.display_name` (42703) — a column absent from both the table and the Prisma model. **6 unscoped queries closed** (`WHERE id = $1` alone, on both reads and writes). **DEF-090 still OPEN:** agent CREATION remains broken, same missing columns in two other files — deliberately left for the owner's migration decision. | **PROGRESS** |
...
|---|---|---|---|
| **`guard-destroys-the-answer`** | **4 in one day, 2026-08-10** | DEF-107 (×2 live), DEF-114, DEF-113 | **NEW TAG — and it inverts how this project has been reading its own failures.** Every instance: Brain reasoned CORRECTLY (`decision=answer`, confidence 0.9–1.0, full history, tool calls with sound rationales) and the answer was destroyed on the last hop. Two capability questions, one apology, and one capability self-report, all replaced by a 107-character canned denial — identified by `answerLen=107` matching the canned string's exact length. **The owner's complaints of "it denied it", "it asks confirmation for nothing", "brain got lost" have been read as reasoning failures for weeks. At least four were output-layer failures.** Cause in each: a protection built for a real incident (DEF-041, a false statement sent to a real person) firing on turns where there was nothing to prevent. **Containment:** every empty-promise change must be checked against DEF-041's verbatim sentence and must fail CLOSED on unknown intent — both asserted by tests. **The structural exit** is the one the guard's own comment has named since 2026-07-08: verify a claim against the DISPATCH LEDGER, not a word list. The ledger became real today (DEF-108) but holds 2 rows in 6 hours — too sparse to be an oracle, and matching prose to rows would risk a false claim coincidentally matching an unrelated entry. **Deliberately NOT attempted yet; it needs a denser ledger and identifier-based matching.** |
...
| **`protection-with-two-implementations`** | **6** | DEF-039, DEF-041, DEF-044, DEF-045, **DEF-118**, **DEF-119** | **6th, same day, found while verifying the 5th.** DEF-119: DEF-115's stale-prompt guard read the `open_item_id` COLUMN, and `preactive_due_nudge` writes the id only to `metadata.openItemId` — 0 of 14 in the column. The guard was verified live on a prompt from the one producer that populates the column, so it looked closed while doing nothing for the producer most exposed to staleness. **Two instances in one day, both discovered by checking whether a fix actually reached every path rather than the path it was found on — that check is now the standing containment for this tag.** **5th, 2026-08-11 — and the first where the two halves were a DETECTOR AND ITS OWN REPAIR.** `sweepWikiEmbeddings` selected pages by `embedding_model IS DISTINCT FROM MODEL_GEMINI` and delegated the write to `embedWikiPage`, whose skip condition was `hash matches && embedding not null` — no model. So the sweep found 7,893 stale pages every 50 minutes, repaired none, and filed a provider alarm against a provider that was healthy. **63% of Brain's memory sat on meaningless vectors for 21 hours while the system's own alarm said so hourly and pointed at the wrong component.** The four earlier instances all had a protection duplicated; this one had a PREDICATE duplicated, which is the same defect wearing different clothes — and the containment (one owner, one implementation, CI-enforced) is now asserted by a test that pins both sides of the predicate. **The four instances below were the most expensive shape found on 08-05.** DEF-039: idempotency + artifact ledger existed only inside the branch a new guard bypassed. DEF-041: the empty-promise regex existed twice — the copy on the live path was the weaker one, under a comment asserting the two were in sync and that it was canonical. Both false for months. **A protection with two implementations has one real implementation and one comforting fiction, and the fiction is what gets read during review.** DEF-044: preview renderer names items by title, result renderer prints raw cuids. DEF-045: the target guard runs at dispatch but not at preview, so the same lookup returning null means "show a placeholder" in one place and "refuse" in the other. Containment in every case: one owner, one implementation, CI-enforced. **This is now the dominant defect shape in the system — ahead of the monolith (DEF-036) that I ranked first this morning.** |
...
| **`proactive-message-absent-from-thread`** | 1 | CL-032 / DEF-117 | **NEW TAG, 2026-08-11.** Distinct from `guard-destroys-the-answer`: no protection fired and nothing was overwritten. Brain's own proactively-sent messages were never written to the thread it reads, so a reply to one arrived with its referent missing and Brain resolved the pronoun against the last visible HUMAN turn. **The shape to watch for: a record of an event written to the audit table but not to the table the next decision reads.** DEF-108 was the same shape one table over (sends recorded in `brain_user_messages`, absent from `brain_action_artifacts`), which is why this is worth a tag at one occurrence — the class already has a second member in spirit. Fixed at the single outbound send path; `sent`/`partial` only, so an undelivered message can never become something Brain believes it said |
